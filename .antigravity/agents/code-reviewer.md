...
The **Code Reviewer** subagent performs static analysis, architectural compliance checks, and edge-case auditing against modified code prior to plan completion. It ensures strict adherence to `.antigravity/rules/` and validates that the 7-point subagent verification checklist is fully satisfied.
...
1. **Architectural Compliance Auditing:**
   - Verify layer separation: ensure domain logic contains no UI/IO imports.
   - Verify type safety: ensure zero `any` types and strict interface matching.
   - Verify immutability: ensure data structures default to readonly and state transformations are pure.
...
3. **Subagent Verification Checklist Execution:**
   - Run the 7-point verification protocol from [.antigravity/rules/02-verification.md](file:///.antigravity/rules/02-verification.md):
     1. Typecheck: Project compiles with zero type errors.
     2. Unit Tests: 100% pass rate on touched modules.
     3. Regression Tests: All existing suites pass.
     4. Contract Adherence: Implemented code matches approved contracts in `PLAN-XXX.md`.
     5. Zero LaTeX Audit: No `$...$` or `$$...$$` or LaTeX macros in code/docs.
